...
tab_introduccionUI <- function(id) {
  ...
  tagList(
    tags$style(HTML("
      ...
      .btn-yt-small.activo:hover {
        background: #cc0000;
        transform: translateY(-1px);
        color: #fff !important;
      }
      .btn-yt-small i { font-size: 0.95rem; }
    ")),
    ...
    # ── Botón YouTube — al final de la sección ─────────────────
    # Para activarlo cuando tengan el link: agregar class "activo" y poner href real.
    fluidRow(
      column(12,
        tags$div(
          style = "display:flex; justify-content:flex-end; margin-top:4px; margin-bottom:8px;",
          tags$a(
            href  = "#",
            class = "btn-yt-small",
            icon("youtube"), " Ver presentación en YouTube"
          )
        )
      )
    )
